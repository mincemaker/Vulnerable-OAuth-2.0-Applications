-- Schema for gallery, ported 1:1 from insecureapplication-go/gallery-idp/db/migrations/001-init.sql
-- (same data, no reason to design it differently a second time). The only
-- deliberate omission is the `sessions` table: that exists in the Go port
-- because it hand-rolled its own OAuth server and cookie session, whereas
-- this Node app already gets both from express-session and oauth2orize
-- without touching this database.
--
-- Deliberately vulnerable: see doc/OAuth2_PoC_Verification_Report.md in the
-- parent repo for the vulnerability classes this schema is designed to
-- reproduce.
